...
Each teams name must be a minimum of 3 characters.
    ...
    global team_a
    global team_b
    # Empty string for team name
    team_a['name'] = ''
    while len(team_a['name']) < 3:
        team_a['name'] = input("Enter the name of the first team: ")\
         .strip().title()
    # Empty string for team name
    team_b['name'] = ''
    while len(team_b['name']) < 3:
        team_b['name'] = input("Enter the name of the second team: ")\
         .strip().title()
...
10 overs per team for an innings.
...
Example: 0,2,1,4,W,0
    ...
    for i in range(10):
        while True:
            score_str = input(f"Enter the score for {team_name}'s\
                              over ({i + 1}/10:)\n")
            ...
            if validate_data(score_over):
                ...
                # Data to append to worksheet
                final_score_over = [team_name] + [i+1] + score_over +\
                    [total_over] + [wickets_lost]
...
def calculate_total_over(total, team_name):
    ...
    """
    # Convert to a list of integers, ignoring non-integer values
    int_data = []
    for num in total:
        try:
...
def calculate_wickets_lost(data, team_name):
    ...
    count = 0
    for item in data:
        # Count occurrences of 'W' in the string and add to the total count
        count += item.count('w') + item.count('W')
    # Appends wickets lost data to dictionary
    if team_name == team_a['name']:
...
def final_score_append(team_name):
    ...
    if team_name['name'] == team_a['name']:
        ...
        # Append header row to worksheet
        team_a_final = ['', '', '', '', '', '', '', 'Final Total',
                        end_score_a, end_wickets_a]
        ...
        # Append header row to worksheet
        team_b_final = ['', '', '', '', '', '', '', 'Final Total',
                        end_score_b, end_wickets_b]
...
def main():
    ...
    calculate_winner()
    # Creates space below scoring before a new game commences scoring
    footer = ['-']
    update_scoresheet(footer, 'Scores')
    footer = ['-']
    update_scoresheet(footer, 'Scores')
    # Ask the user if they want to repeat the program
    repeat = input('Do you want to score another game?\
    Enter "yes" to score again: \n').strip().lower()
    if repeat == 'yes':
        main()
    else:
